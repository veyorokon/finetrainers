...
class Offload:
    ...
    def offload(cls, pipeline: DiffusionPipeline, config: OffloadConfig = OffloadConfig()):
        ...
        print(f"offload models: {print_info} compiler_transformer:{config.compiler_transformer}")
        if config.compiler_transformer:
            ...
            def move_text_encoder_to_gpu(pipe):
                torch.cuda.empty_cache()
                for k, model in pipe.components.items():
                    if isinstance(model, torch.nn.Module) and k not in ["transformer", "vae"]:
                        model.to("cuda")
