...
```shell
python3 test_lora_inference.py --prompt "A girl is riding a bike." --model_path "MODEL_IDENTIFIER" --lora_path "path/to/lora" --lora_name "lora_adapter" --output_file "output.mp4" --fps 8
```
...
def generate_video(model_path, prompt, lora_path, lora_name, output_file, fps):
    # Import the appropriate pipeline class based on model_path
    from diffusers import DiffusionPipeline
    
    pipe = DiffusionPipeline.from_pretrained(model_path, torch_dtype=torch.bfloat16).to("cuda")
...
def main():
    parser = argparse.ArgumentParser(description="Generate video using LoRA weights")
